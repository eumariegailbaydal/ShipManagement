...
export default function CrewPage() {
  ...
  const [currentShipByCrewId, setCurrentShipByCrewId] = useState<Record<string, ShipOption>>({});
  ...
  async function load() {
    ...
    const { data: assignmentData } = await supabase
      ...
      .select("crew_id, sign_on_date, ships(id, name)")
      .is("sign_off_date", null)
      .order("sign_on_date", { ascending: false });
    const map: Record<string, ShipOption> = {};
    (assignmentData as any[] ?? []).forEach((a) => {
      if (a.ships && !map[a.crew_id]) map[a.crew_id] = a.ships;
    });
    setCurrentShipByCrewId(map);
  }
  ...
  const ranks = Array.from(new Set(crew.map((c) => c.rank).filter((r): r is string => !!r))).sort();

  const filteredCrew = crew.filter((c) => {
    if (rankFilter !== "all" && c.rank !== rankFilter) return false;
    if (vesselFilter === "unassigned" && currentShipByCrewId[c.id]) return false;
    if (vesselFilter !== "all" && vesselFilter !== "unassigned" && currentShipByCrewId[c.id]?.id !== vesselFilter) return false;
    return true;
  });
    ...
    <AppShell>
      <div className="p-8 max-w-6xl">
        ...
        <div className="flex items-center gap-3 mb-4">
          <div>
            <label className="block text-xs text-ink/50 mb-1">Filter by rank</label>
            <select
              value={rankFilter}
              onChange={(e) => setRankFilter(e.target.value)}
              className="text-sm border border-ink/20 rounded-sm px-3 py-1.5 bg-transparent"
            >
          ...
          <div>
            <label className="block text-xs text-ink/50 mb-1">Filter by vessel</label>
            <select
              value={vesselFilter}
          ...
          {(rankFilter !== "all" || vesselFilter !== "all") && (
            <button
              onClick={() => {
                setRankFilter("all");
                setVesselFilter("all");
              }}
              className="text-xs text-harbor-700 hover:underline mt-4"
            >
              Clear filters
            </button>
          )}
        ...
        <div className="space-y-2">
            ...
            <div key={c.id} className="panel rounded-sm overflow-hidden">
              <button onClick={() => toggleExpand(c.id)} className="w-full text-left px-4 py-3 flex items-center gap-4 hover:bg-paper/50">
                ...
                <div className="flex-1 grid grid-cols-5 gap-4 items-center">
                  ...
                  <p className="text-sm text-ink/60">{currentShipByCrewId[c.id]?.name ?? "Unassigned"}</p>
          ...
          {filteredCrew.length === 0 && (
            <div className="panel rounded-sm p-8 text-center text-ink/50 text-sm">
              {crew.length === 0 ? "No crew records yet — add your first one above." : "No crew members match this filter."}
            </div>
          )}
        </div>
      </div>
